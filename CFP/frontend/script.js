...
function getFallbackApiBaseUrl() {
  ...
  return isLocalDev ? 'http://localhost:5000/api' : `${window.location.origin}/api`;
}

const API_URL = window.CFPAppConfig?.getApiBaseUrl?.() || getFallbackApiBaseUrl();
...
function prefillContactInquiry(type, message) {
  const typeSelect = document.getElementById('contactType');
  const messageBox = document.getElementById('contactMessage');
  if (typeSelect && type) typeSelect.value = type;
  if (messageBox && message && !messageBox.value.trim()) messageBox.value = message;
}
...
function updateNavForLoggedInUser() {
  ...
  if (user) {
    actions.innerHTML = `
      ...
      <a href="#membership" class="btn-outline">Membership</a>
...
async function checkoutCart() {
  ...
  const legacyTotal = cart.reduce((s, i) => s + i.price * i.qty, 0);
  closeModal('cartModal');
  window._checkoutTotal = legacyTotal;
  window._checkoutType = 'product';
  document.getElementById('paymentPlan').textContent = 'Shop Order';
  document.getElementById('paymentAmount').textContent = `Rs. ${legacyTotal.toLocaleString()}`;
...
function openBooking(className) {
  prefillContactInquiry('Personal Training', `I want to ask about ${className}. Please share schedule and availability.`);
  goToSection('#contact');
  showToast(`Please contact the gym desk to reserve ${className}.`);
  return;
  if (!localStorage.getItem('cfp_token')) {
...
async function processPaymentLegacy(method) {
  const token = localStorage.getItem('cfp_token');
...
function processPayment(method) {
  ...
  if (method === 'Cash') {
    prefillContactInquiry(type === 'membership' ? 'Membership' : 'Shop Order', `I want to confirm counter payment for ${label}. Amount: Rs. ${amount.toLocaleString()}.`);
    goToSection('#contact');
...
async function submitContactLegacy(e) {
  e.preventDefault();
  const form = e.target;
